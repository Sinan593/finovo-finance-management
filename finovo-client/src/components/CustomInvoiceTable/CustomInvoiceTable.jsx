...
export default function CustomInvoiceTable({
  invoiceItems,
  setInvoiceItems,
  fetchInvoiceItems,
  invoiceId,
    ...
    const loadInvoices = async () => {
      try {
        ...
        if (response.data.success) {
          const invoicesList = response.data.data.invoiceItems;
          console.log(`The invoice items is ${JSON.stringify(invoicesList)}`);
          setInvoiceItems(invoicesList);
        }
      } catch (error) {
        console.log(
          `Error while fetching products inside CustomInvoiceTable: ${error}`
        );
      }
    };
  ...
  const refreshInvoiceItems = async () => {
    try {
      ...
      if (response.data.success) {
        const invoicesList = response.data.data.invoiceItems;
        console.log(JSON.stringify(invoicesList));
        setInvoiceItems(invoicesList);
      }
    } catch (error) {
      console.log(
